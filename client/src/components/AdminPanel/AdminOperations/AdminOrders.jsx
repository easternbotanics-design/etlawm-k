  ...
  const columns = [
    {
      ...
      render: (order) => (
        <div>
          ...
          {order.coupon_code && (
            <span
              className="flex items-center gap-1 mt-2 w-fit font-mono text-[10px] font-bold text-[#A77C6B] bg-[#FAF6F0] border border-[#EBE3D5] rounded-md px-2 py-0.5 uppercase tracking-wide"
    ...
    },
    {
      ...
      render: (order) => {
        if (!order.razorpay_payment_id) {
          return <span className="text-xs text-[#7C7770] italic">—</span>;
        }
        if (order.razorpay_payment_id === "payment failed") {
          return (
        ...
        if (order.razorpay_payment_id === "cart abandoned") {
          return (
